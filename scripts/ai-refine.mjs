...
const args = ["--approve", "--no-skills", ...skillArgs, "--provider", "rissets", "--model", model, "--thinking", manifest.ai.thinkingLevel || "high", "--no-session", "-p", "@TENANT_AGENT_BRIEF.md", "Follow the explicitly loaded skills in order. UI UX Pro Max must determine the initial category-specific design system, Taste must produce TASTE_AUDIT.md, Superpowers must produce IMPLEMENTATION_PLAN.md before edits, and threejs-animation must produce MOTION_DECISION.json. Read tenant-onboarding.json, research-dossier.json, design-direction.json, src/generated/tenant-profile.ts, and TENANT_AGENT_BRIEF.md. Materially rewrite src/tenant/** plus supporting React/Tailwind source so navigation, hero, proof/USP, testimonials, CTA, gallery, collection/detail pages, footer, typography, image language, responsive structure, and motion match this business and are unlike a shared template. Add three purposeful reduced-motion-safe SVG systems or a justified lazy Three.js scene. Preserve every public API route and both locales. Run npm run check, fix failures, and write AI_GENERATION_REPORT.md plus generation-report.json. JSON/config-only, palette-only, and edits outside the tenant presentation boundary fail."];
